...
- **Operations Rationale**: Historical records show that 60.8% of past field visits (382 of 628) were false alarms where no fault was found (`Kein Fehler gefunden`). Only 218 visits involved confirmed component repairs (power supplies, antennas, swapped gateways). Equating every historical visit to a confirmed failure would severely miscalibrate dispatch priorities. Crucially, field visits represent an operational selection bias (only suspected gateways were inspected; silently broken gateways lack visit records). All reported figures were dynamically recomputed from the supplied data; the field-visit evaluation is explicitly treated as a selection-biased retrospective proxy. The €600/week penalty delta measures avoided operational losses on confirmed hardware episodes under this internal retrospective proxy, while the forgone lead-time of retrospective detection is transparently acknowledged in `LIMITATIONS.md`.
...
- **Operations Rationale**: Candidate `v0002` achieved an aggregate 15.49% reduction in missed broken weeks on the development fleet under our internal retrospective `COST_BACKTEST` proxy (71 missed in active vs 60 in candidate, saving €6,600 in simulated retrospective fault penalties with zero window regressions; this measures internal backtest proxy performance, not official fleet ground truth). However, on the 59 unseen holdout gateways, candidate `v0002` regressed (17 missed in active vs 18 in candidate). The promotion policy strictly failed closed with `REJECT_GROUPED_DISAGREEMENT`, keeping production safely anchored on `v0001`. For operational agility, rollback is achieved in $<1$ second via atomic pointer swap in `registry/active.json`, restoring exact bit-for-bit replay equality.
- **Authoritative Separation: Real Candidate vs. Rollback Rehearsal Fixture**:
  - **REAL CANDIDATE (`v0002`)**: Evaluated against actual challenge data. Development fleet improved 71 → 60 missed weeks, but the 59-gateway unseen holdout regressed 17 → 18 missed weeks. The promotion gate issued an authoritative `REJECT_GROUPED_DISAGREEMENT`, and production remained strictly locked on active `v0001`.
  - **ROLLBACK FIXTURE (`v_promotable`)**: A deterministic rollback-demo fixture whose promotion decision is intentionally preconstructed for lifecycle rehearsal. Its synthetic evaluation numbers (e.g. synthetic holdout 17 → 14) are **not production model performance**, **do not represent the official hidden-ground-truth score**, and **are not evidence that `v0002` should be promoted**. It exists exclusively to enable end-to-end rehearsal and demonstration of atomic promotion and atomic rollback (`make rollback`, `make ops-rollback-to`) without falsely deploying an unvetted model or violating production governance rules.
