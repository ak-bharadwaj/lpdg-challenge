...
- **Rationale**:
  ...
  - **Operational Limitations Disclosed**: Evaluation scope is `precision_biased_sample` (FAQ 4.4: only suspected gateways were visited; no row exists for quietly broken unvisited gateways). All reported figures were dynamically recomputed from the supplied data; the field-visit evaluation is explicitly treated as a selection-biased retrospective proxy. System detects already-degraded gateways retrospectively; early forecasting lead time is noted as a future enhancement.
  - **Pre-Cutoff Meter-Read Snapshot**: The most recent available pre-cutoff week is `2026-01-26` (`meter_read_success.csv` ends on 2026-01-26; no 2026-02-02 meter data exists). Dispatched gateways exhibited a 66.87% mean success rate vs. 85.63% for the rest of the fleet (an 18.76 percentage-point lower mean meter-read success).
  - **Hardware Fleet Profile**: Fleet model distribution was audited from raw master data. Specifically, model `GW-8800X` comprises 1 gateway, 5 field visits, and 3 confirmed repairs (never phrased as a 300% repair rate).
  - **Engineer Review Subset**: Of the 120 dispatched gateways, exactly 27 were present in the independent post-cutoff sample (`engineer_review_2026-02.xlsx`). Among these 27, 12 were rated *Schlecht* (confirmed hardware defect) and 3 were noted *nach Tausch stabil* (recent physical replacement), totaling 15 / 27 = 55.6% of the reviewed subset (not a fleet-wide confirmation).
...
- **Rationale**:
  - **Empirical Gate Results**:
    - **Temporal Windows (Development Fleet)**: Active missed 71 vs Candidate missed 60 broken weeks (-11 missed weeks = 15.49% aggregate internal retrospective cost proxy reduction, clearing the 10.0% threshold). This metric serves strictly as an internal retrospective `COST_BACKTEST` proxy on historical development periods, not official hidden-ground-truth performance. Individual windows: Nov 32 -> 26 (+18.75%), Dec 24 -> 20 (+16.67%), Jan 15 -> 14 (+6.67%), with zero window regressions.
  ...
  - **Operations-Manager Economic Defense**:
    - **Dispatch Economics**: Technician visit allocation is €45,600.00 fixed (120 visits × €380, constant across all valid submissions). Missed-fault penalties differ under the retrospective backtest proxy: active €88,200 vs candidate €81,600 (an internal €6,600 penalty savings proxy on historical development data).
